...
import { initSearch } from './js/search.js?v=11';
...
async function refreshSessionUi(nextSession) { session = nextSession; await loadMembership(session); renderSessionUi(); }
function bindGlobalUi() {
  document.querySelectorAll('[data-close]').forEach((button) => { button.onclick = () => button.closest('.modal')?.classList.remove('open'); });
  $('homeBtn')?.addEventListener('click', () => window.scrollTo({ top: 0, behavior: 'smooth' }));
}
function bindLegalUi() {
  const legal = {
    privacy: { title: 'Política de Privacidade', body: 'O Chama O Pro utiliza os dados necessários para criar contas, apresentar perfis profissionais, guardar pedidos e melhorar a pesquisa de serviços. A autenticação e a base de dados usam infraestrutura Supabase. A localização indicada é usada para calcular proximidade entre pedidos e profissionais. Não vendemos dados pessoais a anunciantes.' },
...
async function init() {
  bindGlobalUi(); bindLegalUi(); initRequests();
  ...
  session = await initAuth({
    ...
    onIntentReady: async (intent, nextSession) => { await refreshSessionUi(nextSession); if (intent === 'pro') await openProfessionalProfile(); if (intent === 'request') openRequest(); },
    onToast: toast,
  });
  const [searchData] = await Promise.all([initSearch({ getSession, onRequest: requestService }), loadPlan()]);
  initProfessionals({ skills: searchData.skills, onMembershipChange: refreshSessionUi });
  await loadMembership(session); renderSessionUi();
}
init().catch((error) => { console.error(error); if ($('categoryGrid')) $('categoryGrid').innerHTML = '<div class="loading-card">Não foi possível carregar os serviços. Recarrega a página.</div>'; });

...
function ensurePanel() {
  ...
  if (!shouldMount(host)) return null;
  let panel = $('professionalActivity');
  if (panel && panel.parentElement === host) return panel;
  panel = document.createElement('section');
  panel.id = 'professionalActivity';
  panel.className = 'professional-activity';
...
function render(rows) {
  ...
  panel.innerHTML = `
    <div class="professional-activity-head">
      <div>
        <span class="professional-activity-eyebrow">🔔 ATIVIDADE</span>
        <h3>Notificações profissionais</h3>
        <p>Aqui vês exatamente o que aconteceu e a que pedido pertence.</p>
      </div>
    ...
    <div class="professional-activity-list">
      ${rows.length ? rows.map((item) => {
        ...
        return `<article class="professional-activity-item ${item.read_at ? 'is-read' : 'is-unread'}" data-prof-activity-id="${item.notification_id}">
          ...
          <div class="professional-activity-copy">
            <div class="professional-activity-title-row">
              <strong>${escapeHtml(item.title || 'Atualização')}</strong>
              ${item.read_at ? '' : '<span class="professional-activity-new">Nova</span>'}
            </div>
            ${item.body ? `<p>${escapeHtml(item.body)}</p>` : ''}
            <small>${escapeHtml(context)} · ${escapeHtml(stamp(item.created_at))}</small>
          </div>
          <div class="professional-activity-actions">
            ${item.request_id ? `<button class="professional-activity-open" type="button" data-prof-activity-open="${item.notification_id}" data-request-id="${item.request_id}">Ver pedido</button>` : ''}
            ${item.read_at ? '' : `<button class="professional-activity-read" type="button" data-prof-activity-read="${item.notification_id}">Marcar como lida</button>`}
          </div>
        </article>`;
...
async function openActivity(notificationId, requestId) {
  ...
  try {
    ...
    console.error(error);
    showToast('O pedido foi marcado como lido, mas não foi possível abrir a conversa.');
  }
}
...
function bindClicks() {
  document.addEventListener('click', (event) => {
    ...
    if (read) {
      markRead(read.dataset.profActivityRead).then(() => loadActivity()).catch(console.error);
      return;
    }
    ...
    if (open) {
      openActivity(open.dataset.profActivityOpen, open.dataset.requestId).catch(console.error);
      return;
    }
    if (event.target.closest?.('#navPro,#proCta,[data-account-tab="professional"]')) {
      setTimeout(() => loadActivity().catch(console.error), 220);
    }
  });
}
...
async function startRealtime() {
  ...
  channel = S.channel(`professional-activity-${session.user.id}-${Date.now()}`)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'service_notifications' }, () => loadActivity().catch(() => {}))
    .subscribe();
  timer = setInterval(() => {
    if ($('professionalActivity')) loadActivity().catch(() => {});
  }, 30000);
}
...
async function init() {
  ...
  S.auth.onAuthStateChange((_event, nextSession) => {
    session = nextSession;
    setTimeout(() => startRealtime().catch(console.error), 0);
  });
}

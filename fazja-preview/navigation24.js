/* Build 24 — navegação de secções como páginas e estado ativo coerente. */
(() => {
  const $ = (id) => document.getElementById(id);
  const accountModal = $('accountModal');
  const authBtn = $('authBtn');
  const navIds = ['navHome', 'navRequests', 'navPro', 'navAccount'];
  const sectionByNav = { navRequests: 'requests', navPro: 'professional', navAccount: 'profile' };
  const navBySection = { requests: 'navRequests', professional: 'navPro', profile: 'navAccount' };
  const titleBySection = { requests: 'Pedidos', professional: 'Área profissional', profile: 'A minha conta' };
    ...
    button.addEventListener('click', () => {
      ...
      setTimeout(() => {
        if (!applyPage(section)) {
          /* Sem sessão, account.js abre o ecrã de autenticação. Não deixamos uma secção falsa marcada. */
  ...
  setActiveNav('navHome');
  normalizeAuthLabel();
  if (initialSection && navBySection[initialSection]) {
    setTimeout(() => $(navBySection[initialSection])?.click(), 0);
  }
})();

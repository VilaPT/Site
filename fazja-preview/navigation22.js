/* Build 22 — navegação de secções como páginas e estado ativo coerente. */
(() => {
  ...
  navIds.forEach((id) => {
    ...
    button.addEventListener('click', () => {
      ...
      setActiveNav(id);
      setTimeout(() => applyPage(section), 0);
    });
  });
  ...
  if (authBtn) {
    normalizeAuthLabel();
    new MutationObserver(normalizeAuthLabel).observe(authBtn, {
      childList: true,
      characterData: true,
      subtree: true,
    });
